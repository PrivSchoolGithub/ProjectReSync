...
return function()
	SyncAPI('Jump').Create({
		Description = 'Makes the specified user(s) jump. If no user is specified, you will jump.',
		PermissionLevel = 1,
		Shorthand = 'Up',
		...
		Run = function(main,user,users)
			...
			return true,'Made '..table.concat(list,', ')..' jump.',8,users,user.DisplayName..' has made you jump.',8
		end,
	})
end
