--[[
    ...
    Description             Displays hint to everyone in all servers
    --------------------------------------------------------------------------------------
...
return function()
	...
	local function try()
		subSuccess,subscribeConnection = pcall(function()
			return messageService:SubscribeAsync('RS_SysHints',function(receiving)
	...
	SyncAPI('SystemHint').Create({
		Description = 'Broadcasts a system-wide hint to everyone in ALL servers for twenty-five seconds.',
		PermissionLevel = 2,
		Shorthand = {'SH','BroadcastHint','BcH'},
		Parameters = 'message',
		Run = function(main,user,...)
			local sending = table.concat({...},' ') -- Cannot use ... outside vararg func
			local success,issue = pcall(function()
				messageService:PublishAsync('RS_SysHints',{
					Admin = user.DisplayName,
					Message = sending,
				})
			end)
			if success then
				return true,'Broadcasting your hint to all servers now. (Give it a moment)',10
			else
