...
local luaK = {}
local luaP = require(script.Parent.MachineOpcodes)
local luaX = require(script.Parent.LexicalAnalyzer)
...
function luaK:codeAsBx(fs, o, A, sBx)
	return self:codeABx(fs, o, A, sBx + luaP.Limits.sBx)
end
...
-- * used in luaK:discharge2reg(), (lparser) luaY:AdjustAssignment()
------------------------------------------------------------------------
    ...
    if fs.pc == 0 then  -- function start?
      ...
      local previous = fs.f.code[fs.pc - 1]
			if luaP:Get_Opcode(previous) == "OP_LOADNIL" then
				local pfrom = luaP:Get_Argument_A(previous)
				local pto = luaP:Get_Argument_B(previous)
        if pfrom <= from and from <= pto + 1 then  -- can connect both?
          if from + n - 1 > pto then
						luaP:Set_Argument_B(previous, from + n - 1)
          end
          return
        end
      end
...
-- * used in luaY:CloseFunction(), luaY:ReturnStatement()
-- NOTE FROM SCOOT: Used in CloseFunction()??????
------------------------------------------------------------------------
...
function luaK:fixjump(fs, pc, dest)
  ...
  assert(dest ~= self.NO_JUMP)
	if math.abs(offset) > luaP.Limits.sBx then
    luaX:SyntaxError(fs.ls, "control structure too long")
  end
  luaP:Set_Argument_sBx(jmp, offset)
end
...
function luaK:getjump(fs, pc)
  local offset = luaP:Get_Argument_sBx(fs.f.code[pc])
...
function luaK:patchtestreg(fs, node, reg)
  local i = self:getjumpcontrol(fs, node)
	...
	if reg ~= luaP.InvalidRegister and reg ~= luaP:Get_Argument_B(i) then
		luaP:Set_Argument_A(i, reg)
    ...
    -- so the following is required; there is no change to ARG_C
		...
		local b = luaP:Get_Argument_B(i)
		luaP:Set_Argument_A(i, b)
    luaP:Set_Argument_B(i, 0)
...
function luaK:removevalues(fs, list)
  while list ~= self.NO_JUMP do
		self:patchtestreg(fs, list, luaP.InvalidRegister)
    list = self:getjump(fs, list)
  end
end
...
function luaK:dischargejpc(fs)
	self:patchlistaux(fs, fs.jpc, fs.pc, luaP.InvalidRegister, fs.pc)
  fs.jpc = self.NO_JUMP
end

------------------------------------------------------------------------
--
-- * used in (lparser) luaY:WhileStatement(), luaY:RepeatStatement(), luaY:ForBody()
------------------------------------------------------------------------
function luaK:patchlist(fs, list, target)
  if target == fs.pc then
    ...
    assert(target < fs.pc)
		self:patchlistaux(fs, list, target, luaP.InvalidRegister, target)
  end
end
...
function luaK:freereg(fs, reg)
	if not luaP:TestConstant(reg) and reg >= fs.nactvar then
...
function luaK:addk(fs, k, v)
  ...
  if self:ttisnumber(idx) then
    ...
    luaY:GrowVector(L, f.k, fs.nk, f.sizek, nil,
                    luaP.Limits.Bx, "constant table overflow")
...
-- * used in (lparser) luaY:CodeString(), luaY:SingleVariable()
------------------------------------------------------------------------
...
-- * used in (lparser) luaY:SimpleExpression(), luaY:ForNumber()
------------------------------------------------------------------------
...
------------------------------------------------------------------------
--
-- * used in luaK:setmultret(), (lparser) luaY:AdjustAssignment()
------------------------------------------------------------------------
function luaK:setreturns(fs, e, nresults)
  if e.k == "VCALL" then  -- expression is an open function call?
		luaP:Set_Argument_C(self:getcode(fs, e), nresults + 1)
  elseif e.k == "VVARARG" then
		luaP:Set_Argument_B(self:getcode(fs, e), nresults + 1);
		luaP:Set_Argument_A(self:getcode(fs, e), fs.freereg);
...
------------------------------------------------------------------------
--
-- * used in luaK:dischargevars(), (lparser) luaY:AssignVariable()
------------------------------------------------------------------------
function luaK:setoneret(fs, e)
  if e.k == "VCALL" then  -- expression is an open function call?
    e.k = "VNONRELOC"
		e.info = luaP:Get_Argument_A(self:getcode(fs, e))
  elseif e.k == "VVARARG" then
		luaP:Set_Argument_B(self:getcode(fs, e), 2)
...
-- * used in (lparser) luaY:TableIndex()
------------------------------------------------------------------------
...
function luaK:exp2RK(fs, e)
  ...
  if k == "VKNUM" or k == "VTRUE" or k == "VFALSE" or k == "VNIL" then
      ...
      e.k = "VK"
			return luaP:CodeConstantIndexAsRK(e.info)
    end
  elseif k == "VK" then
...
------------------------------------------------------------------------
--
-- * used in (lparser) luaY:AssignVariable(), luaY:LocalFunction(), luaY:FunctionStatement()
------------------------------------------------------------------------
...
------------------------------------------------------------------------
--
-- * used only in (lparser) luaY:PrimaryExpression()
------------------------------------------------------------------------
...
function luaK:invertjump(fs, e)
  local pc = self:getjumpcontrol(fs, e.info)
	...
	luaP:Set_Argument_A(pc, (luaP:Get_Argument_A(pc) == 0) and 1 or 0)
end
  ...
  if e.k == "VRELOCABLE" then
    local ie = self:getcode(fs, e)
      ...
      fs.pc = fs.pc - 1  -- remove previous OP_NOT
			return self:condjump(fs, "OP_TEST", luaP:Get_Argument_B(ie), 0, cond and 0 or 1)
    end
    -- else go through
  end
  self:discharge2anyreg(fs, e)
  self:freeexp(fs, e)
	return self:condjump(fs, "OP_TESTSET", luaP.InvalidRegister, e.info, cond and 1 or 0)
end

------------------------------------------------------------------------
--
-- * used in luaK:infix(), (lparser) luaY:Condition()
------------------------------------------------------------------------
...
------------------------------------------------------------------------
--
-- * used in (lparser) luaY:FunctionName(), luaY:PrimaryExpression()
------------------------------------------------------------------------
...
------------------------------------------------------------------------
--
-- * used only in (lparser) luaY:SubExpression()
------------------------------------------------------------------------
  ...
  elseif op == "OPR_CONCAT" then
    self:exp2val(fs, e2)
		if e2.k == "VRELOCABLE" and luaP:Get_Opcode(self:getcode(fs, e2)) == "OP_CONCAT" then
			assert(e1.info == luaP:Get_Argument_B(self:getcode(fs, e2)) - 1)
      self:freeexp(fs, e1)
			luaP:Set_Argument_B(self:getcode(fs, e2), e1.info)
      e1.k = "VRELOCABLE"
      e1.info = e2.info
    else
...
-- * used in (lparser) luaY:FunctionParameters(), luaY:ForBody(), luaY:FunctionStatement()
------------------------------------------------------------------------
...
-- * called directly by (lparser) luaY:WhileStatement()
------------------------------------------------------------------------
...
function luaK:codeABC(fs, o, a, b, c)
	assert(luaP:GetOpMode(o) == luaP.Format.iABC)
	assert(luaP:GetBMode(o) ~= luaP.OpArgumentMask.OAM_N or b == 0)
	assert(luaP:GetCMode(o) ~= luaP.OpArgumentMask.OAM_N or c == 0)
...
function luaK:codeABx(fs, o, a, bc)
	assert(luaP:GetOpMode(o) == luaP.Format.iABx or
		luaP:GetOpMode(o) == luaP.Format.iAsBx)
	assert(luaP:GetCMode(o) == luaP.OpArgumentMask.OAM_N)
...
function luaK:setlist(fs, base, nelems, tostore)
  ...
  assert(tostore ~= 0)
	if c <= luaP.Limits.C then
    self:codeABC(fs, "OP_SETLIST", base, b, c)
  else
    self:codeABC(fs, "OP_SETLIST", base, b, 0)
    self:code(fs, luaP:Create_Instruction(c), fs.ls.lastline)
  end
