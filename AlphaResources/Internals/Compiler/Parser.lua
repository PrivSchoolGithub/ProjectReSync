...
local luaK = require(script.Parent.CodeGenerator)(luaY)
...
function luaY:PErr_Limit(functionState,limit,issue)
	...
	luaX:lexerror(functionState.ls,message,0)
end
...
function luaY:TErr_Token(luaState,token)
	luaX:syntaxerror(luaState,string.format(self.LUA_QS..' expected',luaX:token2str(luaState,token)))
end
...
function luaY:VerifyConditions(luaState,what,who,where)
	if not self:TestNextToken(luaState,what) then
		if where == luaState.linenumber then
			...
			luaX:syntaxerror(luaState,string.format(self.LUA_QS..' expected (to close'..self.LUA_QS..' at line %d)',luaX:token2str(luaState,what),luaX:token2str(luaState,who),where))
		end
	end
end
...
function luaY:ProcessValues(functionState,number,variable,base)
	if functionState == nil then -- No more levels?
		self:InitializeExpDesc(variable,'VGLOBAL',luaP.NO_REG) -- Default is global variable
		return 'VGLOBAL'
	else
...
function luaY:PushClosure(luaState,run,expression)
	local functionState = luaState.fs
	local header = functionState.f
	self:GrowVector(luaState.L,header.p,functionState.np,header.sizep,nil,luaP.MAXARG_sBx,'constant table overflow')
...
function luaY:Constructor(luaState,t)
	...
	luaP:SETARG_B(functionState.f.code[pc],self:IntegerToFPB(constructControl.na)) -- Set initial array size
	luaP:SETARG_C(functionState.f.code[pc],self:IntegerToFPB(constructControl.nh)) -- Set initial table size
end
...
function luaY:FunctionCall(luaState) -- Changed from "exprstat"
	...
	if leftHandSideAssignmentChain.v.k == 'VCALL' then -- stat -> func
		luaP:SETARG_C(luaK:getcode(functionState,leftHandSideAssignmentChain.v),1) -- Call statement uses no results
	...
	if self:BlockFollow(luaState.t.token) or luaState.t.token == ';' then
		...
		if self:CheckMultipleReturnValues(expressionDescriptionDataStructure.k) then
			...
			if expressionDescriptionDataStructure.k == 'VCALL' and numberOfRegisters == 1 then -- Tail call?
				...
				assert(luaP:GETARG_A(luaK:getcode(functionState,expressionDescriptionDataStructure)) == functionState.nactvar)
			end
...
-- * luaY.SHRT_MAX (from <limits.h>) for RegisterLocalVariable
...
-- * break keyword no longer skipped in luaY:BreakStatement
-- * function NewLocalVariablestr replaced by NewLocalVariableLiteral
-- * RegisterLocalVariable limits local variables to SHRT_MAX
-- * create_local deleted, NewLocalVariableLiteral used instead
-- * constant LUAI_MAXUPVALUES increased to 60
-- * constants MAXPARAMS, LUA_MAXPARSERLEVEL, MAXSTACK removed
-- * function interface changed: ProcessValues, SingleVariable
-- * EnterLevel and LeaveLevel uses nCcalls to track call depth
-- * added a name argument to main entry function, luaY:parser
-- * function luaY_index changed to yindex changed to TableIndex
-- * luaY:int2fb()'s table size encoding format has been changed
-- * luaY:log2() no longer needed for table constructors
-- * function code_params deleted, functionality folded in Parameters
-- * vararg flags handling (is_vararg) changes; also see VARARG_*
-- * LUA_COMPATUPSYNTAX section for old-style upvalues removed
-- * RepeatStatement() calls Chunk() instead of block()
-- * function interface changed: cond, TestThenBlock
